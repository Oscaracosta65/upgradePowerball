...
function leFmtDateLong(?string $date): string
{
    ...
    return ($ts === false) ? "\xe2\x80\x94" : date('F j, Y', $ts);
}
...
$rowsPB   = ($nodCurrentPB === $nodCurrentMain) ? $rowsMain : leFetchRecentDraws($db, (string) $dbCol, $gameId, $nodCurrentPB);

[$mainCounts, $mainLastSeenIndex] = leInitRange(1, 69);
[$pbCounts, $pbLastSeenIndex]     = leInitRange(1, 26);
...
foreach ($rowsPB as $idx => $row) {
    $ball = trim((string) ($row['sixth'] ?? ''));

    if ($ball === '' || !isset($pbCounts[$ball])) {
        continue;
    }

    $pbCounts[$ball]++;

    if ($pbLastSeenIndex[$ball] === null) {
        $pbLastSeenIndex[$ball] = (int) $idx;
    }
}

$mainChartLabels = leBuildNaturalLabels(1, 69);
$mainChartValues = [];
$mainRecencyValues = [];
...
$pbChartLabels = leBuildNaturalLabels(1, 26);
$pbChartValues = [];

foreach ($pbChartLabels as $label) {
    $pbChartValues[] = (int) ($pbCounts[$label] ?? 0);
}
...
[$counts50, ]  = leInitRange(1, 69);
[$counts300, ] = leInitRange(1, 69);
...
.skai-ball--main{
  background:linear-gradient(180deg, #FFFFFF 0%, #F3F6FF 100%);color:var(--deep-navy);
  border:1px solid rgba(10,26,51,.14);box-shadow:0 10px 20px rgba(10,26,51,.12), inset 0 1px 0 rgba(255,255,255,.90);
}

.skai-ball--bonus{
  background:radial-gradient(circle at 50% 18%, #C73E4E 0%, #8F1F2D 76%, #4A0911 100%);color:#fff;
  border:1px solid rgba(255,255,255,.16);box-shadow:0 12px 24px rgba(10,26,51,.18);
}
...
.skai-btn{
  display:inline-flex;align-items:center;justify-content:center;gap:10px;border-radius:14px;
...
.skai-mini-link{
  display:flex;align-items:center;justify-content:center;text-align:center;min-height:44px;padding:10px 12px;
...
.skai-chart-frame{position:relative;width:100%;height:300px}
.skai-chart-frame--tall{height:1120px}
...
<div class="skai-page">
    ...
    <div class="skai-hero-inner">
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          <div class="skai-kicker">Results Intelligence &#8226; Verified Draw &#8226; Calm Analytical View</div>
          <h1 class="skai-title">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=101">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=101">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=101">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=101">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gameId=101">Heatmap Analysis</a>
          </div>
        </div>
          ...
          <div class="skai-meta-stack">
            ...
            <?php if ($nextJackpot !== '') : ?>
              <div class="skai-meta-box">
                ...
                <span class="value"><?php echo htmlspecialchars((string) $nextJackpot, ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
            <?php endif; ?>
          </div>
        </aside>
  ...
  <section class="skai-strip" aria-label="Key takeaways">
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        <div class="skai-stat-value"><?php echo (int) $nodCurrentMain; ?> / <?php echo (int) $nodCurrentPB; ?></div>
        <div class="skai-stat-note">Main-number and Powerball draw windows currently loaded for this page view.</div>
      </div>
    </article>
  </section>
  ...
  <section id="overview" class="skai-section" aria-labelledby="overview-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">Start with a clear high-level view. This layer is designed for fast orientation: which numbers are most active, which are quiet, and how the current draw relates to recent history.</p>
      </div>
    </div>
      ...
      <div class="skai-overview-grid">
        <div class="skai-card">
          ...
          <div class="skai-card-body">
            <div class="skai-chart-frame"><canvas id="topActiveChart" aria-label="Top active numbers chart" role="img"></canvas></div>
          </div>
        </div>
        <div class="skai-card">
          ...
          <div class="skai-card-body">
            <div class="skai-chart-frame"><canvas id="quietChart" aria-label="Quiet numbers chart" role="img"></canvas></div>
          </div>
        </div>
      </div>
  ...
  <section id="recency-deep-dive" class="skai-section" aria-labelledby="recency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">This section makes the current draw easier to interpret. It shows how recently each latest number was previously seen and how the perspective shifts when you compare a short recent window with a broader historical one.</p>
      </div>
    </div>
    <div class="skai-section-body">
              ...
              <?php foreach ($drawHistoryRows as $row) : ?>
                <div class="skai-history-item">
                  ...
                  <div class="skai-history-badge">
                    <?php echo ($row['drawsAgo'] !== null) ? (int) $row['drawsAgo'] . ' drws ago' : "\xe2\x80\x94"; ?>
                  </div>
                </div>
              <?php endforeach; ?>
            </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">Move from summary to full reference. The first panel below shows the complete main-number distribution across all values 01&ndash;69. The second panel shows the complete Powerball distribution for the selected bonus window.</p>
      </div>
    </div>
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;69 across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-frame skai-chart-frame--tall"><canvas id="fullMainChart" aria-label="Full main number distribution chart" role="img"></canvas></div>
          </div>
        </div>
        <div class="skai-grid">
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--radiant">
              ...
              <span class="skai-card-sub">All values 01&ndash;26 across the last <?php echo (int) $nodCurrentPB; ?> drawings</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-frame"><canvas id="pbChart" aria-label="Powerball distribution chart" role="img"></canvas></div>
            </div>
          </div>
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--ember">
            ...
            <div class="skai-card-body">
              <div class="skai-chart-frame"><canvas id="recencyChart" aria-label="Main numbers recency chart" role="img"></canvas></div>
            </div>
          </div>
        </div>
      </div>
      <div class="skai-note">The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review.</div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">Use the tables for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset.</p>
      </div>
    </div>
    <div class="skai-controls">
      <form name="fqsearch" method="post" action="/powerball-winning-numbers-analysis-tools#tables">
        <div class="skai-controls-row">
          <div class="skai-controls-left">
            ...
            <label for="pnod">Powerball draw window</label>
            <select name="pnod" id="pnod" class="skai-select">
              <?php foreach (range(10, 700, 5) as $opt) : ?>
                <option value="<?php echo (int) $opt; ?>"<?php echo ((int) $opt === (int) $nodCurrentPB) ? ' selected="selected"' : ''; ?>><?php echo (int) $opt; ?></option>
              <?php endforeach; ?>
            </select>
          </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;69</span>
          </div>
          <div class="skai-controls">
            <div class="skai-controls-row">
              <div class="skai-controls-left">
          ...
          <div class="skai-table-wrap">
            <table id="skai-main-table" class="skai-table" aria-label="Main numbers frequency table">
              <thead>
                <tr><th>Number</th><th>Drawn Times</th><th>Last Drawn</th><th>Track</th></tr>
              </thead>
              <tbody>
                <?php for ($i = 1; $i <= 69; $i++) : ?>
                  <?php
                  ...
                  $rowClass = 'all';
                  if (in_array($number, $topActiveKeys, true)) { $rowClass .= ' active'; }
                  if (in_array($number, $quietestKeys, true)) { $rowClass .= ' quiet'; }
                  if (($mainLastSeenIndex[$number] ?? null) !== null && (int) $mainLastSeenIndex[$number] <= 4) { $rowClass .= ' recent'; }
                  ?>
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            Powerball table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;26</span>
          </div>
          <div class="skai-table-wrap">
            <table id="skai-pb-table" class="skai-table" aria-label="Powerball frequency table">
              <thead>
                <tr><th>Number</th><th>Drawn Times</th><th>Last Drawn</th><th>Track</th></tr>
              </thead>
              <tbody>
                <?php for ($i = 1; $i <= 26; $i++) : ?>
                  <?php
                  $number = ($i < 10) ? '0' . $i : (string) $i;
                  $countNumber = (int) ($pbCounts[$number] ?? 0);
                  [$lastDrawSort, $lastDrawLabel] = leDrawingsAgoLabel($pbLastSeenIndex[$number] ?? null, (int) $nodCurrentPB);
                  ?>
                  <tr>
                    <td><span class="skai-pill skai-pill--bonus"><?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?></span></td>
                    <td><?php echo (int) $countNumber; ?> X</td>
                    <td data-sort="<?php echo (int) $lastDrawSort; ?>"><?php echo htmlspecialchars($lastDrawLabel, ENT_QUOTES, 'UTF-8'); ?></td>
                    <td><input class="skai-checkbox js-track-bonus" type="checkbox" value="<?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>" aria-label="Track Powerball number <?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"></td>
                  </tr>
                <?php endfor; ?>
              </tbody>
            </table>
          ...
          <div class="skai-tracked">
            <div class="skai-tracked-head">
              <h3 class="skai-tracked-title">Tracked Powerball values</h3>
          ...
          <div class="skai-note" style="margin-top:14px;">Tracking is local to this page view. It is intended as a lightweight comparison aid while you move between the overview, tables, and advanced SKAI tools.</div>
        </div>
      </div>
    </div>
  </section>
  ...
  <section id="tools" class="skai-section" aria-labelledby="tools-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">The results page establishes context. These tools take that context into deeper modeling and structured exploration.</p>
      </div>
    </div>
    <div class="skai-section-body">
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            <p class="skai-tool-copy">Best next step for a broader multi-signal view. Use this after reviewing frequency and recency to move into the main SKAI intelligence workflow.</p>
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=101">Open SKAI Analysis</a>
          </div>
        </article>
        <article class="skai-tool">
          <div class="skai-tool-head skai-card-head--radiant">AI Predictions</div>
          <div class="skai-tool-body">
            <p class="skai-tool-copy">Use when you want a model-driven complement to the historical view shown on this page.</p>
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=101">Open AI Predictions</a>
          </div>
        </article>
        <article class="skai-tool">
          <div class="skai-tool-head skai-card-head--success">Skip &amp; Hit Analysis</div>
          <div class="skai-tool-body">
            <p class="skai-tool-copy">Useful for users who want to compare appearance spacing and interruption behavior after reviewing current frequency.</p>
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=101">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>
      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=101">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gameId=101">Heatmap Analysis</a>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">This page is designed to help users understand recent and historical behavior more clearly, not to imply certainty.</p>
      </div>
    </div>
    <div class="skai-section-body">
...
(function () {
  ...
  var chartData = {
    ...
    pbLabels: <?php echo json_encode(array_values($pbChartLabels)); ?>,
    pbValues: <?php echo json_encode(array_values($pbChartValues)); ?>
  };
  ...
  function renderCharts() {
    if (!window.Chart || _chartsRendered) {
      return;
    }
    ...
    var pbCanvas = document.getElementById('pbChart');
    ...
    if (topActiveCanvas && topActiveCanvas.getContext) {
      new Chart(topActiveCanvas.getContext('2d'), {
        ...
        data: {
          ...
          datasets: [{ data: chartData.topActiveValues, borderWidth: 0, borderRadius: 8, backgroundColor: '#1C66FF' }]
        },
    ...
    if (quietCanvas && quietCanvas.getContext) {
      new Chart(quietCanvas.getContext('2d'), {
        ...
        data: {
          ...
          datasets: [{ data: chartData.quietValues, borderWidth: 0, borderRadius: 8, backgroundColor: '#F5A623' }]
        },
        options: commonBarOptions(false)
      });
    }
    ...
    if (fullMainCanvas && fullMainCanvas.getContext) {
      new Chart(fullMainCanvas.getContext('2d'), {
        ...
        data: {
          ...
          datasets: [{ data: chartData.mainValues, borderWidth: 0, borderRadius: 6, barThickness: 10, maxBarThickness: 12, backgroundColor: '#1C66FF' }]
        },
    ...
    if (pbCanvas && pbCanvas.getContext) {
      new Chart(pbCanvas.getContext('2d'), {
        type: 'bar',
        data: {
          labels: chartData.pbLabels,
          datasets: [{ data: chartData.pbValues, borderWidth: 0, borderRadius: 8, backgroundColor: '#7F8DAA' }]
        },
        options: commonBarOptions(false)
      });
    }
    ...
    if (recencyCanvas && recencyCanvas.getContext) {
      new Chart(recencyCanvas.getContext('2d'), {
        ...
        data: {
          labels: chartData.mainLabels,
          datasets: [{ data: chartData.mainRecencyValues, borderWidth: 0, borderRadius: 6, backgroundColor: '#F5A623' }]
        },
        options: {
          responsive: true,
          maintainAspectRatio: false,
          ...
          scales: {
            x: { ticks: { maxRotation: 0, autoSkip: true, font: { weight: '700' } }, grid: { display: false } },
  ...
  function bindFilters() {
    ...
    function applyFilter(filter) {
      ...
      for (var j = 0; j < buttons.length; j++) {
        ...
        if (buttons[j].getAttribute('data-filter') === filter) { buttons[j].classList.add('is-active'); }
      }
    }
  ...
  function initChartWithRetry() {
    ...
    function tryRender() {
      ...
      attempts++;
      if (attempts < maxAttempts) { setTimeout(tryRender, 800); }
    }

    loadChartJsIfNeeded(function () { tryRender(); });
  }
